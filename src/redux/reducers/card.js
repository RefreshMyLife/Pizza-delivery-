...
const card = (state = initialState, action) =>{
    switch (action.type) {
        ...
        case 'MINUS_CART_ITEM': {
            const oldItems = state.items[action.payload].item;
            const newObjItems =
                oldItems.length > 1 ? state.items[action.payload].item.slice(1) : oldItems;
            const newItems = {
                ...state.items,
                [action.payload]: {
                    item: newObjItems,
